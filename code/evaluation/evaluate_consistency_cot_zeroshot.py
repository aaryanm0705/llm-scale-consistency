#!/usr/bin/env python3
"""
Chain-of-Thought Zero-Shot Evaluation for LLM Consistency

Hybrid CoT strategy: Llama and Mistral receive JSON-format prompts; remaining
four models receive natural-reasoning prompts with regex extraction.

Methodology: Free-form generation (max_new_tokens=512), deterministic decoding
Output: consistency_results_cot_zeroshot.csv
"""
...
OUTPUT_FILE = os.path.join(RESULTS_DIR, "consistency_results_cot_zeroshot.csv")
...
def create_cot_zeroshot_prompt(question_text, options_list, model_name):
    """Build CoT zero-shot prompt using hybrid JSON/natural-reasoning strategy. Returns (messages, valid_letters)."""
...
def generate_answer(model, tokenizer, question_text, options_list, model_name):
    """Generate CoT response and extract answer. Returns (letter, reasoning, raw_response, is_valid, extraction_method)."""
    messages, valid_letters = create_cot_zeroshot_prompt(question_text, options_list, model_name)
    try:
...
def evaluate_model(model_name, test_df):
    """Evaluate single model on all question variants. Returns list of result dicts."""
    ...
    for idx, row in test_df.iterrows():
        ...
        answer_letter, reasoning, raw_response, is_valid, extraction_method = generate_answer(
            model, tokenizer, row['question'], options_list, model_name
        )
...
def main():
    print(f"CoT Zero-Shot Evaluation | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
