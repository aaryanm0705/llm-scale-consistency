#!/usr/bin/env python3
"""
Chain-of-Thought One-Shot Evaluation for LLM Consistency

Hybrid CoT strategy with one-shot exemplar: variant-1 answer text from the
zeroshot baseline is shown to all 5 variants as prior context.

DEPENDENCY: Requires consistency_results_cot_zeroshot.csv

Methodology: Free-form generation (max_new_tokens=512), deterministic decoding
Output: consistency_results_cot_oneshot.csv
"""
...
BASELINE_FILE = os.path.join(RESULTS_DIR, "consistency_results_cot_zeroshot.csv")
OUTPUT_FILE = os.path.join(RESULTS_DIR, "consistency_results_cot_oneshot.csv")
...
def create_cot_oneshot_prompt(question_text, options_list, exemplar_text, model_name):
    """Build CoT one-shot prompt with exemplar. Returns (messages, valid_letters)."""
...
def generate_answer(model, tokenizer, question_text, options_list, exemplar_text, model_name):
    """Generate CoT response with one-shot exemplar. Returns (letter, reasoning, raw_response, is_valid, extraction_method)."""
    messages, valid_letters = create_cot_oneshot_prompt(
        question_text, options_list, exemplar_text, model_name
    )
    try:
...
def evaluate_model(model_name, test_df, baseline_answers):
    """Evaluate single model on all 5 variants with one-shot exemplar. Returns list of result dicts."""
    ...
    for idx, row in test_df.iterrows():
        ...
        if question_id not in baseline_answers:
            print(f"Warning: No baseline for {question_id}, skipping...")
            continue
        ...
        answer_letter, reasoning, raw_response, is_valid, extraction_method = generate_answer(
            model, tokenizer, row['question'], options_list, exemplar_text, model_name
        )
...
def main():
    if not os.path.exists(BASELINE_FILE):
        ...
        print(f"Please run CoT zero-shot evaluation first.")
        sys.exit(1)

    print(f"CoT One-Shot Evaluation | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    for model_name in MODELS_TO_EVALUATE:
        ...
        baseline_answers[model_name] = dict(zip(
    ...
    # Splice variant-1 rows from CoT zero-shot baseline to produce complete 5-variant output
    baseline_df_full = pd.read_csv(BASELINE_FILE)
    for model_name in MODELS_TO_EVALUATE:
        zeroshot_var1 = baseline_df_full[
            (baseline_df_full['model'] == model_name) &
            (baseline_df_full['answer_var_id'] == 1)
        ]
        print(f"  Adding {len(zeroshot_var1)} variant-1 rows for {model_name}")
        for _, row in zeroshot_var1.iterrows():
            all_results.append({
                ...
                'reasoning_length': row.get('reasoning_length', 0),
