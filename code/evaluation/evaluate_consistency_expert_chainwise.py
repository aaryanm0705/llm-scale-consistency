#!/usr/bin/env python3
"""
Expert Sequential Evaluation for LLM Consistency

Evaluates all 5 variants per question under expert framing with cumulative context.
Each variant receives all prior variants' answer texts alongside the thesis context.

Methodology: Token-level enforcement, deterministic generation
Output: consistency_results_expert_sequential.csv
"""
...
OUTPUT_FILE = os.path.join(RESULTS_DIR, "consistency_results_expert_sequential.csv")
...
def create_expert_sequential_prompt(question_text, options_list, previous_answers=None):
    """Create expert sequential prompt with thesis context and cumulative prior answers. Returns (messages, valid_letters)."""
...
def generate_answer(model, tokenizer, question_text, options_list, previous_answers=None):
    ...
    messages, valid_letters = create_expert_sequential_prompt(question_text, options_list, previous_answers)
    try:
...
def main():
    print(f"Expert Sequential Evaluation | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
