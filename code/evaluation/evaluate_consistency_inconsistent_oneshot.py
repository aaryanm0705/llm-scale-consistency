#!/usr/bin/env python3
"""
Inconsistent One-Shot Evaluation for LLM Consistency

Control condition: models receive variant-1 answer as exemplar and are instructed
to select a different position, demonstrating the upper bound of prompt-driven
inconsistency with a single prior example.

Methodology: Token-level enforcement, deterministic generation
Output: consistency_results_inconsistent_oneshot.csv
"""
...
BASELINE_FILE = os.path.join(RESULTS_DIR, "consistency_results_inconsistent_zeroshot.csv")
OUTPUT_FILE = os.path.join(RESULTS_DIR, "consistency_results_inconsistent_oneshot.csv")
...
def create_inconsistent_oneshot_prompt(question_text, options_list, exemplar_text):
    """Build inconsistent one-shot prompt with exemplar. Returns (messages, valid_letters)."""
...
Instructions:
- Your previous answer was "{exemplar_text}"
- IMPORTANT: Select a DIFFERENT position from your previous answer
...
def generate_answer(model, tokenizer, question_text, options_list, exemplar_text):
    ...
    messages, valid_letters = create_inconsistent_oneshot_prompt(
        question_text, options_list, exemplar_text
    )
    try:
...
def main():
    print(f"Inconsistent One-Shot Evaluation | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    # Splice variant-1 rows from zeroshot baseline to produce complete 5-variant output
