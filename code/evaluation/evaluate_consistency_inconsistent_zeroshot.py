#!/usr/bin/env python3
"""
Inconsistent Zero-Shot Evaluation for LLM Consistency

Control condition: models are instructed to deliberately vary responses across
question variations, serving as a lower bound on achievable consistency.

Methodology: Token-level enforcement, deterministic generation
Output: consistency_results_inconsistent_zeroshot.csv
"""
...
OUTPUT_FILE = os.path.join(RESULTS_DIR, "consistency_results_inconsistent_zeroshot.csv")
...
def create_inconsistent_zeroshot_prompt(question_text, options_list):
    """Build inconsistent zero-shot prompt. Returns (messages, valid_letters)."""
...
Instructions:
- Deliberately vary your response across question variations
...
def generate_answer(model, tokenizer, question_text, options_list):
    ...
    messages, valid_letters = create_inconsistent_zeroshot_prompt(question_text, options_list)
    try:
...
def main():
    print(f"Inconsistent Zero-Shot Evaluation | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
